        ...
        private async Task CheckConnectionAsync(Connection connection)
{
    ...
    try
    {
        // Если нет соединения с античитом — просто логируем и выходим
        if (!IsConnected)
        {
            LogPlayerEvent(connection.userid, "skipped anti-cheat check (WebSocket offline)");
            return;
        }
    ...
    try
    {
        switch (command)
        {
            ...
            case KickCommand:
                HandleKick(values);
                break;
            case "DISCONNECT_MESSAGE":  // Добавить эту строку
                HandleDisconnect(values);  // Использовать существующий метод
                break;
            default:
